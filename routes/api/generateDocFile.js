...
router.post("/", async (req, res) => {
  ...
  const filePath1 = path.join(
    os.homedir(),
    "Downloads",
    "Initial Intake(answer and question).docx"
  );
  let fileIndex1 = 0;
  let currentFilePath1 = filePath1;
  while (fs.existsSync(currentFilePath1)) {
    fileIndex1++;
  ...
  Packer.toBuffer(doc).then((buffer) => {
    fs.writeFileSync(currentFilePath1, buffer);
  });

  const filePath = path.join(
    os.homedir(),
    "Downloads",
    "Initial Intake(story).docx"
  );
  let fileIndex = 0;
  ...
  Packer.toBuffer(storyDoc).then((buffer) => {
