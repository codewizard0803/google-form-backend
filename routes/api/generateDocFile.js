...
router.post("/", async (req, res) => {
 ...
 return  value.map((item, index) => 
      `${index + 1}. ` +  Object.keys(item).map(p =>  `${p}` + ":" + `${item[p]}`) + `${'\n'}`
    ).join("")
  }
    ...
    sections: [
      {
        children: [
          ...
          req.body?.employmentInjuryPhysicalValue?.radioDisabilityConnectionClaimItem === "Yes"
          ...
          req.body?.employmentInjuryPhysicalValue?.relationshipImmediateSupervisor === "Poor"
